<p><h1 align="center" style="color:gold"> HOLIDAYS <br>
</h1>
<br>
<br>
<h2 align="center">
<a href="https://fabricmc.net"><img src="https://cdn.discordapp.com/attachments/705864145169416313/969720133998239794/fabric_supported.png" width="200"></a> <br>
<a href="https://github.com/Illyena/Holidays/blob/1.18.2/LICENSE"><img src="https:shields.io/github/license/Illyena/Holidays"></a>
</h2>

## Description:
A special something for a special day.

### Authors:
[Illyena](https://github.com/Illyena)

## Setup:
![Mod loader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)
![environment](https:img.shields.io/badge/environment-both-4caf50)

Please install an instance of Fabric Loader for your client (and the server if playing multiplayer).
### Dependencies:
<p><a href="https://modrinth.com/mod/fabric-api"><img src="https://i.imgur.com/Ol1Tcf8.png" width="200"></a></p>
This mod requires you to also install:

-[Fabric API](https://modrinth.com/mod/fabric-api) <br>
-[Gilding-core](https://modrinth.com/mod/Gilding-core) <br>


### Please report any issues to the GitHub repository
[![issues](https://shields.io/github/issues/Illyena/Holidays)](https://github.com/Illyena/Holidays/issues)

### For Developers:
[![jitpack](https://jitpack.io/v/Illyena/Holidays.svg)](https://jitpack.io/#Illyena/Holidays) <br>
add the following to your `build.gradle` file
```gradle
repositories {
    ...
    
	maven { url = "https://jitpack.io" }
}
dependencies {
    ...
    
    modRuntimeOnly "com.github.Illyena:Gilding-core-Api-mc${project.minecraft_version}:${project.core_api_version}"
    modImplementation "com.github.Illyena:Gilding-core-mc${project.minecraft_version}:${project.gilding_version}"
    modImplementation "com.github.Illyena:Holidays-mc${project.minecraft_version}:${project.holidays_version}"
}
```

add the following to your `gradle.properties` file
```gradle
#Dependencies
    ...
    
    core_api_version=0.0.1
    gilding_version=0.3.0
    holidays_version=0.1.0
```


### License
This mod is available under the [![GitHub](https://img.shields.io/github/license/Illyena/Holidays)](https://github.com/Illyena/Holidays/blob/1.18.2/LICENSE)
